function getAbout(json:any) {
    const aboutComponent = json?.included?.find((d : any) =>
  ...
  function getExperience(json:any) {
  ...
  function getVolunteering(json: any) {
    const volunteeringEntity = json?.included?.find((d: any) =>
      d?.entityUrn?.includes("VOLUNTEERING_EXPERIENCE")
    );
  ...
  function getEducation(json: any) {
    const educationComponent = json?.included?.find((d: any) => {
  ...
  async function getMiddleProfile(profileId: any) {
    try {
  ...
  async function getTopProfile(handle: any) {
    try {
      const res = await fetch(
      ...
      const entityWithAllTheData = data?.included?.find(
        (d: any) => d?.publicIdentifier && d?.publicIdentifier !== "adrianhorning"
      );
  ...
  export async function getLinkedinPage(handle: any) {
